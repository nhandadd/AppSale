...
public class HomeViewModel extends ViewModel {
    ...
    private MutableLiveData<AppResource<Order>> cartData = new MutableLiveData<>();
    ...
    public LiveData<AppResource<Order>> getCart() {
        return cartData;
    }
    ...
    public void fetchCart(String idOrder) {
        cartData.setValue(new AppResource.Loading(null));
        Call<AppResource<Order>> callOrder = orderRepository.getCarts(idOrder);
        callOrder.enqueue(new Callback<AppResource<Order>>() {
            @Override
            public void onResponse(Call<AppResource<Order>> call, Response<AppResource<Order>> response) {
                if (response.isSuccessful()) {
                    AppResource<Order> orderResponse = response.body();
                    if (orderResponse.data != null) {
                        cartData.setValue(new AppResource.Success<>(orderResponse.data));
                    }
                } else {
                    try {
                        JSONObject jsonObject = new JSONObject(response.errorBody().string());
                        String message = jsonObject.getString("message");
                        cartData.setValue(new AppResource.Error<>(message));
                    } catch (JSONException e) {
                        e.printStackTrace();
                    } catch (IOException e) {
                        e.printStackTrace();
            ...
            @Override
            public void onFailure(Call<AppResource<Order>> call, Throwable t) {
                cartData.setValue(new AppResource.Error<>(t.getMessage()));
            }
        });
    }
}
